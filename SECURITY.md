...
- Codex / Claude browser authorization uses PKCE S256, a cryptographically random state, a 15-minute in-memory session, and one-time session consumption.
- CLIProxyAPI / sub2api JSON is parsed only in authenticated administrator endpoints. Import preview is masked and does not return Access Token, Refresh Token, ID Token, or the original JSON. No account is selected by default.
