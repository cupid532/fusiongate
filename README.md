...
- 公共模型 / 别名与多条候选路由；候选可暂停/恢复、拖动排序，并可按公开模型独立选择“优先级故障转移”“顺序轮询”或“智能择优”。
...
同一公开模型可以配置多个上游候选。候选可随时**暂停/恢复**而无需删除，拖动顺序会持久化，并同时作为同级故障转移顺序或轮询顺序。每个公开模型可独立选择：

- **优先级故障转移**（默认）：路由优先级数字越大越先使用，`0` 最低；同一优先级按页面顺序依次尝试。当前候选失败、熔断或达到并发上限时，继续切换到下一候选。
- **顺序轮询**：忽略优先级，按页面顺序让新请求逐个轮换；单个请求若失败，仍会沿当前顺序继续故障转移。
- **智能择优**：根据 Provider 权重、EWMA 延迟、连续失败、当前并发和熔断状态动态分配请求，稳定且低延迟的候选会获得更多流量。健康状态当前按 Provider 聚合。

熔断中的渠道、正在执行半开探针的渠道，以及达到最大并发的渠道都会被排除。每一次故障转移都会写入独立 attempt，并保留上一跳失败原因。Provider 页面中的“智能权重”仅用于智能择优；“管理排序”只控制渠道列表展示，不等同于模型路由优先级。
