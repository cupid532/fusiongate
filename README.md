...
- 请求账本实时显示进行中请求、逐秒读数的动态运行时间（区分“等待首字节”与“输出中”）、Token 明细（输入/缓存/推理/输出）、每次故障转移尝试及上游首字节耗时；滞留的进行中记录会被标记为“疑强停滞”，启动与周期清扫会自动关账遗留的开行，避免僵尸记录永久显示进行中。
...
- 在渠道页选择一种全局起始渠道策略（四种策略都带同样的请求内故障转移与熔断保护，区别只在每个新请求的起点怎么选）：**优先级固定**（默认，总从最高优先级开始）、**配置顺序固定**（总从列表第一个开始）、**渠道间轮换**（每个新请求换下一个可用渠道，平均分摊）或**自适应加权**（按权重、首字节延迟、失败历史和当前并发打分选起点）。
