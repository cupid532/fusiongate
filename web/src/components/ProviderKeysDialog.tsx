...
import type { IPPoolNode, ProviderKey } from "@/lib/types"
...
export function ProviderKeysPanel({ open, providerId, onManageModels, onClose }: PanelProps) {
  const qc = useQueryClient()
  const confirmDelete = useConfirmDelete()
  const [newKey, setNewKey] = useState("")
  const [newName, setNewName] = useState("")
  const [error, setError] = useState("")
  useEffect(() => { setNewKey(""); setNewName(""); setError("") }, [providerId])
  const { data: nodes = [] } = useQuery({ queryKey: ["ippool"], queryFn: () => api<IPPoolNode[]>("/api/admin/ip-pool"), enabled: open })
  const { data: keys = [], isLoading, isError, error: loadError } = useQuery({
  ...
  return <div className="space-y-4">
      ...
      {isLoading ? <div className="py-6 text-center text-sm text-muted-foreground">加载中…</div> : isError ? <div role="alert" className="py-6 text-center text-sm text-destructive">{loadError instanceof Error ? loadError.message : "读取 Key 失败"}</div> : keys.length === 0 ? <div className="py-6 text-center text-sm text-muted-foreground">还没有 Key</div> : <div className="space-y-2">{keys.map((key: ProviderKey) => <section key={key.id} className="rounded-md border p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-medium">{key.name || "API Key"}</span><span className="font-mono text-xs text-muted-foreground">{key.key_hint}</span>{statusBadge(key.status)}</div><div className="mt-1 text-xs text-muted-foreground">模型 {key.models?.length ?? 0} 个 · 上次测试 {key.last_tested_at ? `${key.last_test_latency_ms} ms` : "尚未测试"}</div>{key.last_error && <div className="mt-1 break-words text-xs text-destructive">{key.last_error}</div>}</div><div className="flex flex-wrap items-center gap-3"><label className="flex items-center gap-1.5 text-xs text-muted-foreground">启用<Switch checked={key.enabled} disabled={patch.isPending} onCheckedChange={(enabled) => patch.mutate({ keyId: key.id, body: { enabled } })} aria-label={`${key.name || key.key_hint} 启用`} /></label><label className="flex items-center gap-1.5 text-xs text-muted-foreground">检活<Switch checked={key.health_check_enabled} disabled={patch.isPending} onCheckedChange={(health_check_enabled) => patch.mutate({ keyId: key.id, body: { health_check_enabled } })} aria-label={`${key.name || key.key_hint} 检活`} /></label><Button variant="outline" size="sm" onClick={() => test.mutate(key.id)} disabled={test.isPending}><Activity />{test.isPending ? "测试中…" : "测试"}</Button><Button variant="ghost" size="icon" title="删除 Key" aria-label={`删除 ${key.name || key.key_hint}`} disabled={remove.isPending} onClick={async () => { if (await confirmDelete(`Key ${key.name || key.key_hint}`)) remove.mutate(key.id) }}><Trash2 className="text-destructive" /></Button></div></div>
        <div className="mt-3 grid gap-2 border-t pt-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_9rem]">
          <div className="space-y-1"><Label htmlFor={`key-name-${key.id}`}>Key 名称</Label><Input id={`key-name-${key.id}`} defaultValue={key.name || ""} onBlur={(event) => { const name = event.target.value.trim(); if (name !== (key.name || "")) patch.mutate({ keyId: key.id, body: { name } }) }} /></div>
          <div className="space-y-1"><Label htmlFor={`key-egress-${key.id}`}>出口</Label><select id={`key-egress-${key.id}`} value={key.egress_mode === "node" ? `node:${key.ip_pool_node_id}` : key.egress_mode || "inherit"} onChange={(event) => { const [mode, node] = event.target.value.split(":"); patch.mutate({ keyId: key.id, body: { egress_mode: mode, ip_pool_node_id: mode === "node" ? Number(node) : null } }) }} className="h-9 w-full rounded-md border border-input bg-transparent px-2 text-sm"><option value="inherit">继承渠道出口</option><option value="direct">直连</option>{nodes.map((node) => <option key={node.id} value={`node:${node.id}`}>节点：{node.name}</option>)}</select></div>
          <div className="space-y-1"><Label htmlFor={`key-cost-${key.id}`}>成本倍率</Label><Input id={`key-cost-${key.id}`} type="number" min="0.01" max="1000" step="0.01" defaultValue={key.cost_multiplier || 1} onBlur={(event) => { const value = Number(event.target.value); if (value > 0 && value <= 1000 && value !== (key.cost_multiplier || 1)) patch.mutate({ keyId: key.id, body: { cost_multiplier: value } }) }} /></div>
        </div>
      </section>)}</div>}
...
export function ProviderKeysDialog({ open, onOpenChange, providerId, providerName, onManageModels }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
      ...
      <ProviderKeysPanel open={open} providerId={providerId} onManageModels={onManageModels} onClose={() => onOpenChange(false)} />
    </DialogContent>
  </Dialog>
}
