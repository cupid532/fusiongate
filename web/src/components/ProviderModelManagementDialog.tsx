...
export function ProviderModelsPanel({ open, providerId, onClose, onDirtyChange }: PanelProps) {
  ...
  const dirty = keys.some((key) => !sameDraft(drafts[key.id] ?? draftFor(key), draftFor(key)))
...
export function ProviderModelManagementDialog({ open, onOpenChange, providerId, providerName }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex max-h-[90vh] max-w-5xl flex-col overflow-hidden">
      ...
      <ProviderModelsPanel open={open} providerId={providerId} onClose={() => onOpenChange(false)} />
    </DialogContent>
  </Dialog>
}
