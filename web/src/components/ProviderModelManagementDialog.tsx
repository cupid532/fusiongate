...
export function ProviderModelManagementDialog({ open, onOpenChange, providerId, providerName }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] max-w-5xl overflow-hidden">
      ...
      {isLoading ? <div className="py-8 text-center text-sm text-muted-foreground">加载中…</div> : keys.length === 0 ? <div className="py-8 text-center text-sm text-muted-foreground">还没有 API Key，请先在 Key 管理中添加。</div> : <div className="grid min-h-0 gap-4 md:grid-cols-[13rem_minmax(0,1fr)]">
