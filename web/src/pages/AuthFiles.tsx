...
export function AuthFiles() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">认证账户</CardTitle>
          <CardDescription>{oauth.length} 个 OAuth 凭据。</CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-8 text-center text-sm text-muted-foreground">加载中…</div>
          ) : oauth.length === 0 ? (
            <div className="p-8 text-center text-sm text-muted-foreground">还没有认证文件，点击「导入文件」或「添加授权」。</div>
          ) : (
