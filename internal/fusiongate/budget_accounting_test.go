...
func settleSpend(a *App, keyID int64, requestID string, cost int64) {
	...
	a.endLedger(attemptID, route.Provider.ID, keyID, "openai", route.Route.UpstreamModel, true, 200, "", time.Now(), Usage{CostMicros: cost, CostType: "estimated", Reported: true})
	a.flushLedgerWrites()
}
