...
func TestResolvePoolsRoutesByFinalUpstreamModel(t *testing.T) {
  ...
  if err != nil { t.Fatal(err) }
  if len(routes) != 3 { t.Fatalf("routes=%d, want 3: %#v", len(routes), routes) }
  seen := map[int64]string{}
  for _, route := range routes {
    if route.Route.PublicName != "gpt-5.6-luna" { t.Fatalf("public name=%q", route.Route.PublicName) }
    seen[route.Provider.ID] = route.Route.UpstreamModel
  }
  if seen[first] != "deepseek-v4-flash" || seen[second] != "deepseek-v4-flash" || seen[other] != "another-upstream" { t.Fatalf("pool members=%v", seen) }
}
...
func TestResolveDoesNotDoubleCountProviderInUpstreamPool(t *testing.T) {
  ...
  routes, err := a.resolve(context.Background(), "gpt-5.6-luna", "chat")
  if err != nil { t.Fatal(err) }
  if len(routes) != 1 { t.Fatalf("provider received %d pool seats, want 1", len(routes)) }
  if routes[0].Route.PublicName != "gpt-5.6-luna" || routes[0].Route.UpstreamModel != "deepseek-v4-flash" { t.Fatalf("route=%+v", routes[0].Route) }
}
...
func TestResolveDoesNotMixUnrelatedUpstreamPools(t *testing.T) {
  ...
  routes, err := a.resolve(context.Background(), "gpt-5.6-luna", "chat")
  if err != nil { t.Fatal(err) }
  if len(routes) != 1 || routes[0].Provider.ID != wanted { t.Fatalf("unrelated route entered pool: %#v", routes) }
}

func insertPoolRoute(t *testing.T, a *App, providerID int64, publicName, upstreamModel string) {
  t.Helper()
  if _, err := a.db.Exec(`INSERT INTO model_routes(public_name,provider_id,upstream_model,capabilities,enabled,priority,sort_order,created_at,updated_at) VALUES(?,?,?,'chat,stream',1,0,0,?,?)`, publicName, providerID, upstreamModel, now(), now()); err != nil { t.Fatal(err) }
}