...
func (a *App) providers(w http.ResponseWriter, r *http.Request, _ adminCtx) {
	...
	case http.MethodPost:
		...
		if strings.TrimSpace(in.KeySelectionMode) == "" {
			in.KeySelectionMode = providerKeySelectionConfigured
		}
		if !validKeySelectionMode(in.KeySelectionMode) {
			fail(w, http.StatusBadRequest, "invalid_request", "key_selection_mode must be configured, low_multiplier, high_multiplier, or round_robin")
			return
		}
		in.KeySelectionMode = normalizeKeySelectionMode(in.KeySelectionMode)
		in.ProtocolPolicy = strings.ToLower(strings.TrimSpace(in.ProtocolPolicy))
		if in.ProtocolPolicy == "" {
			in.ProtocolPolicy = protocolAuto
		}
		...
		if err := validateProviderWebsite(in.WebsiteURL); err != nil {
			fail(w, http.StatusBadRequest, "invalid_website_url", err.Error())
			return
		}
		priority := 1
		if in.Priority != nil {
...
func (a *App) providerUpdate(w http.ResponseWriter, r *http.Request, id int64) {
	...
	connectionEditRequested := in.Name != nil || in.Type != nil || in.BaseURL != nil || in.WebsiteURL != nil || in.Credential != nil
	...
	if in.WebsiteURL != nil {
		value := strings.TrimSpace(*in.WebsiteURL)
		if err := validateProviderWebsite(value); err != nil {
			fail(w, http.StatusBadRequest, "invalid_website_url", err.Error())
			return
		}
		in.WebsiteURL = &value
	...
	if in.KeySelectionMode != nil {
		...
		value := normalizeKeySelectionMode(*in.KeySelectionMode)
		in.KeySelectionMode = &value
	}
	currentProtocolPolicy, currentProtocolPreference := protocolAuto, ""
	if err := a.db.QueryRow(`SELECT protocol_policy,protocol_preference FROM providers WHERE id=?`, id).Scan(&currentProtocolPolicy, &currentProtocolPreference); err != nil {
		fail(w, http.StatusInternalServerError, "database_error", err.Error())
		return
	}
	if in.ProtocolPolicy != nil {
		value := strings.ToLower(strings.TrimSpace(*in.ProtocolPolicy))
		if !validProtocolPolicy(value) {
			fail(w, http.StatusBadRequest, "invalid_request", "protocol_policy must be auto or fixed")
			return
		}
		in.ProtocolPolicy = &value
		currentProtocolPolicy = value
	}
	if in.ProtocolPreference != nil {
		value, valid := normalizeProtocolPreference(*in.ProtocolPreference)
		if !valid {
			fail(w, http.StatusBadRequest, "invalid_request", "protocol_preference accepts responses, messages, and chat")
			return
		}
	...
	connectionChanged := credentialUpdated || (in.Name != nil && *in.Name != currentName) || (in.Type != nil && *in.Type != currentType) || (in.BaseURL != nil && *in.BaseURL != currentBaseURL) || in.IPPoolNodeID != nil || in.DefaultModel != nil || protocolChanged
	discoveryConnectionChanged := credentialUpdated || (in.Type != nil && *in.Type != currentType) || (in.BaseURL != nil && *in.BaseURL != currentBaseURL) || protocolChanged
	if in.IPPoolNodeID != nil && *in.IPPoolNodeID > 0 {
		if err := a.validateIPPoolNode(*in.IPPoolNodeID); err != nil {
			fail(w, http.StatusBadRequest, "invalid_ip_pool_node", err.Error())
			return
	...
	res, err := tx.ExecContext(r.Context(), `UPDATE providers SET name=COALESCE(?,name),type=COALESCE(?,type),base_url=COALESCE(?,base_url),website_url=COALESCE(?,website_url),credential=COALESCE(?,credential),enabled=COALESCE(?,enabled),archived=COALESCE(?,archived),priority=COALESCE(?,priority),weight=COALESCE(?,weight),notes=COALESCE(?,notes),passthrough_mode=COALESCE(?,passthrough_mode),client_policy=COALESCE(?,client_policy),max_concurrency=COALESCE(?,max_concurrency),request_timeout_ms=COALESCE(?,request_timeout_ms),failure_threshold=COALESCE(?,failure_threshold),cooldown_seconds=COALESCE(?,cooldown_seconds),health_check_enabled=COALESCE(?,health_check_enabled),group_id=CASE WHEN ? THEN ? ELSE group_id END,group_sort_order=COALESCE(?,group_sort_order),ip_pool_node_id=CASE WHEN ? THEN ? ELSE ip_pool_node_id END,default_model=COALESCE(?,default_model),key_selection_mode=COALESCE(?,key_selection_mode),protocol_policy=COALESCE(?,protocol_policy),protocol_preference=COALESCE(?,protocol_preference),updated_at=? WHERE id=?`, in.Name, in.Type, in.BaseURL, in.WebsiteURL, encryptedCredential, maybeBool(in.Enabled), maybeBool(in.Archived), in.Priority, in.Weight, in.Notes, in.PassthroughMode, in.ClientPolicy, in.MaxConcurrency, in.RequestTimeoutMS, in.FailureThreshold, in.CooldownSeconds, maybeBool(in.HealthCheckEnabled), groupAssignRequested, groupIDArg, in.GroupSortOrder, in.IPPoolNodeID != nil, ipPoolNodeArg, in.DefaultModel, in.KeySelectionMode, in.ProtocolPolicy, in.ProtocolPreference, now(), id)
	if err != nil {
	...
	response := map[string]any{"ok": true, "credential_updated": credentialUpdated}
	if protocolChanged {
		discovery, discoveryErr := a.discoverProviderModels(r.Context(), id)
		if discoveryErr != nil {
			response["protocol_discovery"] = map[string]any{"status": "failed", "error": sanitizeError(discoveryErr.Error())}
		} else {
			response["protocol_discovery"] = map[string]any{"status": "ok", "discovered": discovery.Discovered}
		}
	}
...
func validateProviderWebsite(raw string) error {
	if raw == "" {
		return nil
	}
	u, err := urlParse(raw)
	if err != nil || (u.Scheme != "http" && u.Scheme != "https") || u.Hostname() == "" {
		return fmt.Errorf("website URL must be an absolute HTTP(S) URL without credentials")
	}
	return nil
}
