...
func TestFailoverRecordsAttempts(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "primary", "openai_compatible", first.URL, "one", 1, 100, "normalized", "any", 0, 3, 30)
	p2 := insertTestProvider(t, a, "backup", "openai_compatible", second.URL, "two", 2, 100, "normalized", "any", 0, 3, 30)
...
func TestStreamingDoesNotFailOverAfterResponseStarts(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "stream-primary", "openai_compatible", first.URL, "one", 1, 1, "normalized", "any", 0, 3, 30)
	p2 := insertTestProvider(t, a, "stream-backup", "openai_compatible", backup.URL, "two", 2, 1, "normalized", "any", 0, 3, 30)
...
func TestStreamingFailsOverBeforeFirstByte(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "empty-primary", "openai_compatible", first.URL, "one", 1, 1, "normalized", "any", 0, 3, 30)
	p2 := insertTestProvider(t, a, "empty-backup", "openai_compatible", backup.URL, "two", 2, 1, "normalized", "any", 0, 3, 30)
	insertTestRoute(t, a, p1, "stream-model", "upstream", "chat,stream", 1)
	insertTestRoute(t, a, p2, "stream-model", "upstream", "chat,stream", 1)
	key := insertTestKey(t, a, false)
	rec := gatewayRequest(t, a, "/v1/chat/completions", key, `{"model":"stream-model","stream":true,"messages":[]}`, "test/1")
...
func TestClientErrorDoesNotFailOver(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "client-error", "openai_compatible", primary.URL, "one", 1, 1, "normalized", "any", 0, 3, 30)
	p2 := insertTestProvider(t, a, "unused-backup", "openai_compatible", backup.URL, "two", 2, 1, "normalized", "any", 0, 3, 30)
...
func TestImageTransportFailureIsNotReplayed(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "image-primary", "openai_compatible", broken.URL, "one", 1, 1, "normalized", "any", 0, 3, 30)
	p2 := insertTestProvider(t, a, "image-backup", "openai_compatible", backup.URL, "two", 2, 1, "normalized", "any", 0, 3, 30)
...
func TestEmptyStreamFailsOverBeforeHeadersAreCommitted(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "empty-stream", "openai_compatible", primary.URL, "one", 1, 1, "normalized", "any", 0, 3, 30)
	p2 := insertTestProvider(t, a, "stream-backup", "openai_compatible", backup.URL, "two", 2, 1, "normalized", "any", 0, 3, 30)
...
func TestRetryAfterImmediatelyOpensProviderCircuit(t *testing.T) {
	...
	defer a.Close()
	p1 := insertTestProvider(t, a, "rate-limited", "openai_compatible", primary.URL, "one", 1, 1, "normalized", "any", 0, 5, 30)
	p2 := insertTestProvider(t, a, "rate-backup", "openai_compatible", backup.URL, "two", 2, 1, "normalized", "any", 0, 5, 30)
	insertTestRoute(t, a, p1, "model", "upstream", "chat", 1)
	insertTestRoute(t, a, p2, "model", "upstream", "chat", 1)
	key := insertTestKey(t, a, false)
